 ...
 * Updated for Vercel Hobby + bdapps IP Whitelist Debugging
 */
...
app.get("/api/v1/health", (req, res) => {
	res.json({
		...
		note: "Use /api/v1/my-ip to check current Vercel outbound IP for bdapps whitelist",
	});
});

// ─── Debug: Current Outbound IP (Very Important for bdapps E1303) ───
app.get("/api/v1/my-ip", (req, res) => {
	...
	console.log(`[IP DEBUG] Vercel outbound IP: ${ip}`);

	res.json({
		statusCode: "S1000",
		statusDetail: "Current Vercel outbound IP for bdapps",
		your_vercel_outbound_ip: ip,
		instruction:
			"1. Copy this IP\n2. Login to bdapps Developer Portal\n3. Go to your App → Allowed Host Addresses / IP Whitelist\n4. Add this IP and save",
		note: "On Vercel Hobby, IPs are dynamic. You may need to add multiple IPs over time if it changes.",
	});
});
...
app.post("/api/v1/sms/send", async (req, res) => {
	try {
		...
		if (deliveryStatusRequest)
			sender.setDeliveryStatusRequest(deliveryStatusRequest);
		...
		res.status(500).json({
			...
			hint:
				error.statusCode === "E1303"
					? "IP not whitelisted in bdapps portal"
					: "Check logs",
		});
	}
});
		...
		if (!sessionId || !message || !destinationAddress) {
			return res
				...
				.json({
					statusCode: "E1312",
					statusDetail:
						"sessionId, message, and destinationAddress are required.",
				});
		}

		const sender = new UssdSender(`${BASE_URL}/ussd/send`, APP_ID, PASSWORD);
		const result = await sender.ussd(
			sessionId,
			message,
			destinationAddress,
...
app.post("/api/v1/ussd/receive", (req, res) => {
	try {
		...
		logger.writeLog(
			`USSD received from ${data.sourceAddress}: ${data.message}`,
		);
		res.json(receiver.getResponse());
	} catch (error) {
	...
	try {
		...
		if (!externalTrxId || !subscriberId || !amount) {
			return res
				...
				.json({
					statusCode: "E1312",
					statusDetail: "externalTrxId, subscriberId, and amount are required.",
				});
		}

		const debitSender = new DirectDebitSender(
			`${BASE_URL}/caas/direct/debit`,
			APP_ID,
			PASSWORD,
		);
		...
		res
			.status(500)
			.json({ statusCode: error.code || "E1601", statusDetail: error.message });
	}
});
...
app.post("/api/v1/caas/balance-query", async (req, res) => {
	try {
		...
		const result = await balanceQuery.queryBalance(
			subscriberId,
...
app.post("/api/v1/subscription/status", async (req, res) => {
	try {
		...
		res.status(500).json({
			statusCode: error.code || "E1601",
			statusDetail: error.message,
			hint:
				error.code === "E1303"
					? "Add current IP from /api/v1/my-ip to bdapps whitelist"
					: "",
		});
	}
});
...
app.post("/api/v1/otp/request", async (req, res) => {
	try {
		const { userMobile, applicationHash, client, device, os, appCode } =
			req.body;
		...
		res.status(500).json({
			statusCode: "E1601",
			statusDetail: error.message,
			hint: error.message?.includes("E1303")
				? "IP not allowed in bdapps. Check /api/v1/my-ip"
				: "",
		});
	}
});
...
app.post("/api/v1/notification", (req, res) => {
	try {
		const { timeStamp, status, applicationId, subscriberId, frequency } =
			req.body;
...
app.listen(PORT, () => {
	console.log(`\n╔══════════════════════════════════════════════════╗`);
	console.log(`║       BDApps API Gateway - Express.js (Vercel)   ║`);
	console.log(`╠══════════════════════════════════════════════════╣`);
	console.log(`║  Server running on port: ${PORT}                     ║`);
	console.log(`║  Health:     GET  /api/v1/health                 ║`);
	console.log(`║  IP Debug:   GET  /api/v1/my-ip   ← Use this!    ║`);
	console.log(
		`║  App ID:     ${APP_ID ? APP_ID.substring(0, 12) + "..." : "NOT SET"}               ║`,
	);
	console.log(`╚══════════════════════════════════════════════════╝\n`);

	if (!APP_ID || !PASSWORD) {
		console.warn(
			"⚠️  WARNING: BDAPPS_APP_ID or BDAPPS_PASSWORD environment variables are missing!",
		);
	}
});
